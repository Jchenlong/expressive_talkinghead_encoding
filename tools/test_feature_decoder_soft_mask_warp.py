...
face_blend_tool.update_blending_params(copy_region="[[274,494,80,432]]",
                                       soft_mask_region="[[309, 364, 152, 395], [329, 383, 159, 409], [347, 400, 161, 409],[358, 422, 167, 411], [378, 441, 174, 392], [397, 457, 182, 373], [412, 472, 189, 351], [426, 491, 204, 332]]")
...
def validate_video_gen(
    ...
    video_cap = cv2.VideoCapture(background_video_path)
    test_video_cap = None
    # test_video_cap = cv2.VideoCapture('/data1/chenlong/0517/video/0822/k6jGR8Ng_re/k6jGR8Ng_re_exp_set_v4_depoly/lm_train_n/results/k6jGR8Ng_re_exp_set_v4_depoly_k6jGR8Ng_re_exp_set_v4_depoly_exp_n_linear.mp4')
    # test_video_cap = cv2.VideoCapture('/data1/chenlong/0517/video/0822/k6jGR8Ng_re/k6jGR8Ng_re_exp_set_v4_depoly/lm_train_n/results/k6jGR8Ng_re_exp_set_v4_depoly_k6jGR8Ng_re_exp_set_v4_depoly_exp_n_linear.mp4')
    test_video_cap = cv2.VideoCapture('/data1/chenlong/0517/video/0822/KMU2dVBV_2_20240103/KMU2dVBV_2_20240103_exp_set_v4_depoly/lm_train_n/results/KMU2dVBV_2_20240103_exp_set_v4_depoly_KMU2dVBV_2_20240103_exp_set_v4_depoly_exp_n_linear.mp4')
    ...
    with imageio.get_writer(save_video_path, fps = 25) as writer:
        for index in tqdm(range(video_length)):
            ...
            # cv2.imwrite(f'/data1/chenlong/yuyuhang/interface_out/1115/1/you/{index}.jpg',image[...,::-1])
            # kernel = np.ones((5, 5), np.uint8)
            # mask = cv2.dilate(mask, kernel)
            if state_dict_path is None:
                success, image = test_video_cap.read()

            # image = merge_from_two_image(image_gt, image[...,::-1], mask=mask, blender=None)

            # face_mask = np.ones_like(image)
            # image = face_blend_tool.blend_with_mask(image_gt, image[...,::-1], image_gt, face_mask)
            ...
            out_im = warp_tool.warp_inverse(background[...,::-1], image, AffineMatrixs[index], image_gt)
...
def expressive_encoding_pipeline(expname,
    ...
    background_video_path = f'/data1/chenlong/online_model_set/video/{expname}/video_split.mp4'
    AffineMatrixs = np.load(f'/data1/chenlong/online_model_set/video/{expname}/video_split/smooth_M.npy')
